...
In our analysis, we discovered an apparent correlation between sleep and activity level. In particular, this was highlighted by our participants improved sleep time on Sunday, with the improved activity level at the beginning of the week. Additionally, we discovered that participants spend too much time in bed, not sleeping. My proposed solution to this, is a new product. The product would be a simplified alarm clock that connects to and can be controlled by the users app. In addition to the functionality from the app, the alarm clock would display the time and have a single button. Users would press this button when they get in bed and press it again when they get out of bed. They would then have access to this data via their app and could see insights into their sleep patterns. This would have a few positive outcomes, the first would be that users would be more mindful of the time they're in bed. Another would be that our data analytics team would have much better quality data to use for our analysis. 
